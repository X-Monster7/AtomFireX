...
def get_transform_K(inputs, is_training, bn_decay = None, K = 3):
    ...
    net = tf_util.conv2d(
        inputs, 256, [1, 1], padding = 'VALID', stride = [1, 1],
    ...
    with tf.variable_scope('transform_feat') as sc:
        ...
        biases = tf.get_variable(
            'biases', [K * K], initializer = tf.constant_initializer(0.0), dtype = tf.float32
            ) + tf.constant(np.eye(K).flatten(), dtype = tf.float32)
...
def get_transform(point_cloud, is_training, bn_decay = None, K = 3):
    ...
    net = tf_util.conv2d(
        input_image, 64, [1, 3], padding = 'VALID', stride = [1, 1],
        bn = True, is_training = is_training, scope = 'tconv1', bn_decay = bn_decay
        )
    net = tf_util.conv2d(
    ...
    with tf.variable_scope('transform_XYZ') as sc:
        ...
        biases = tf.get_variable(
            'biases', [3 * K], initializer = tf.constant_initializer(0.0), dtype = tf.float32
...
def get_model(point_cloud, input_label, is_training, cat_num, part_num, \
    ...
    out1 = tf_util.conv2d(
        input_image, 64, [1, K], padding = 'VALID', stride = [1, 1],
        bn = True, is_training = is_training, scope = 'conv1', bn_decay = bn_decay
        )
    out2 = tf_util.conv2d(
        out1, 128, [1, 1], padding = 'VALID', stride = [1, 1],
        bn = True, is_training = is_training, scope = 'conv2', bn_decay = bn_decay
        )
    out3 = tf_util.conv2d(
        out2, 128, [1, 1], padding = 'VALID', stride = [1, 1],
    ...
    out4 = tf_util.conv2d(
        net_transformed, 512, [1, 1], padding = 'VALID', stride = [1, 1],
        bn = True, is_training = is_training, scope = 'conv4', bn_decay = bn_decay
        )
    out5 = tf_util.conv2d(
        out4, 2048, [1, 1], padding = 'VALID', stride = [1, 1],
    ...
    net2 = tf_util.conv2d(
        concat, 256, [1, 1], padding = 'VALID', stride = [1, 1], bn_decay = bn_decay,
    ...
    net2 = tf_util.conv2d(
        net2, 256, [1, 1], padding = 'VALID', stride = [1, 1], bn_decay = bn_decay,
    ...
    net2 = tf_util.conv2d(
        net2, 128, [1, 1], padding = 'VALID', stride = [1, 1], bn_decay = bn_decay,
        bn = True, is_training = is_training, scope = 'seg/conv3', weight_decay = weight_decay
        )
    net2 = tf_util.conv2d(
        net2, part_num, [1, 1], padding = 'VALID', stride = [1, 1], activation_fn = None,
...
def get_loss(l_pred, seg_pred, label, seg, weight, end_points):
    ...
    mat_diff = tf.matmul(transform, tf.transpose(transform, perm = [0, 2, 1])) - tf.constant(
        np.eye(K), dtype = tf.float32
        )
